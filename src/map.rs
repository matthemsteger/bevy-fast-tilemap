...
pub struct Map {
    ...
    pub(crate) map_bg_colors: Vec<Color>,

    #[storage(104, read_only)]
    pub(crate) map_fg_colors: Vec<Color>,
...
impl<'a> MapIndexer<'a> {
    ...
    pub fn set_fg_color(&mut self, x: u32, y: u32, color: Color) {
        // ensure x/y do not go out of bounds individually (even if the final index is in-bounds)
        if x >= self.size().x || y >= self.size().y {
            return;
        }
        let idx = y as usize * self.size().x as usize + x as usize;
        self.map.map_fg_colors[idx] = color;
    }
}
